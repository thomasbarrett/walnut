...
from walnut.models.qwen3_5 import _copy_weights
...
def test_copy_weights_fills_every_parameter():
    module = _module()
    _copy_weights(module, _weights(module))
...
def test_copy_weights_rejects_an_unfilled_parameter():
    ...
    with pytest.raises(ValueError, match="unfilled"):
        _copy_weights(module, _weights(module)[:1])
...
def test_copy_weights_rejects_an_unmatched_tensor():
    ...
    with pytest.raises(ValueError, match="unmatched"):
        _copy_weights(module, extra)


def test_copy_weights_skips_unimplemented_heads():
    module = _module()
    _copy_weights(module, [*_weights(module), ("mtp.fc.weight", torch.ones(2, 2))])
    assert all((p == 1).all() for p in module.parameters())
