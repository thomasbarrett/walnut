# Appendix B. The complete prelude

Save as `prelude.sql`; run with `tp query -f prelude.sql -f analysis.sql trace.json` or execute statement-by-statement from Python.

```sql
-- ===========================================================================
-- PyTorch inference trace analysis: PerfettoSQL prelude
-- Compatible with Kineto chrome traces from torch.profiler.export_chrome_trace
-- ===========================================================================
...
-- record_function / ProfilerStep markers, indexed per name.
...
-- Built on `flow` (robust to CUDA graphs and to missing correlation ids).
...
[Index](README.md) · [← Appendix A](appendix-a-category-and-arg-reference.md) · [Appendix C →](appendix-c-query-index.md)
